...
describe('DTrackClient', () => {
  ...
  it('should get the dependency track version successfully', async () => {
    // Arrange
    mockAxiosInstance.get.mockResolvedValue({
      status: 200,
      data: { version: '5.0.0' }
  ...
  it('should get a project by name and version successfully', async () => {
    // Arrange
    const projectName = 'ProjectName';
    const projectVersion = '1.0.0';
    const project = { uuid: '123e4567-e89b-12d3-a456-426614174000', name: projectName, version: projectVersion };
    mockAxiosInstance.get.mockResolvedValue({
      status: 200,
      data: project
    });
    ...
    // Act
    const result = await client.getProjectByNameAndVersion(projectName, projectVersion);
  ...
  it('should return null when getting a project by name and version that does not exist', async () => {
    // Arrange
    const error = new Error('Not Found');
    error.response = { status: 404 };
    mockAxiosInstance.get.mockRejectedValue(error);
    ...
    // Act
    const result = await client.getProjectByNameAndVersion('ProjectName', '1.0.0');

    // Assert
    expect(result).toBeNull();
  });

  it('should get the latest project version successfully', async () => {
    // Arrange
    const projectName = 'ProjectName';
    const project = { uuid: '123e4567-e89b-12d3-a456-426614174000', name: projectName, version: '1.0.0' };
    mockAxiosInstance.get.mockResolvedValue({
      status: 200,
      data: project
    });
    ...
    // Act
    const result = await client.getLatestProjectVersion(projectName);

    // Assert
    expect(result).toEqual(project);
    expect(mockAxiosInstance.get).toHaveBeenCalledWith(`/api/v1/project/latest/${projectName}`);
  });

  it('should return null when no latest project version exists', async () => {
    // Arrange
    const error = new Error('Not Found');
    error.response = { status: 404 };
    mockAxiosInstance.get.mockRejectedValue(error);
  ...
  it('should clone a project using the v1 API successfully', async () => {
    // Arrange
    ...
    // Act
    const result = await client.cloneProjectV1Async(projectUuid, '2.0.0', true, includes);

    // Assert
    expect(result).toEqual(token);
    expect(mockAxiosInstance.put).toHaveBeenCalledWith('/api/v1/project/clone', {
      project: projectUuid,
      version: '2.0.0',
  ...
  it('should clone a project using the v2 API successfully', async () => {
    // Arrange
    const projectUuid = '123e4567-e89b-12d3-a456-426614174000';
    ...
    const includes = {
      ...
      policyViolationsAuditHistory: false
    };
    ...
    // Act
    const result = await client.cloneProjectV2Async(projectUuid, '2.0.0', false, includes);

    // Assert
    expect(result).toEqual(newUuid);
    expect(mockAxiosInstance.post).toHaveBeenCalledWith(`/api/v2/projects/${projectUuid}/clone`, {
      version: '2.0.0',
      version_is_latest: false,
      includes: ['COMPONENTS', 'FINDINGS', 'FINDINGS_AUDIT_HISTORY', 'TAGS']
    });
  });
